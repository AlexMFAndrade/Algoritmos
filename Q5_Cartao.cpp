...
//Recebendo Número do Cartão	
	...
	for(int i=0; i<16; i++){
		cartao[i] = numero_cartao[i]-'0';//passando para númerico e jogando em cartão
		//cout<<cartao[i];
	}
	...
	//Validando Digito Verificador
		//Posições Ímpares
			for(int i=0;i<15;i=i+2){
				aux1 = cartao[i]*2;
				if(aux1>=10){
					stringstream aux; //stringstream -> converte numeral em string -- precisa da biblioteca sstream
					string dig_str;
					aux<<aux1;
					aux>>dig_str;
